...
from sqlalchemy import desc, update as sqlalchemy_update, delete as sqlalchemy_delete, func
...
class BaseDAO(Generic[T]):
    ...
    @classmethod
    async def find_all(cls, session: AsyncSession, filters: dict = {}, order = None):
        try:
    ...
    @classmethod
    async def add(cls, session: AsyncSession, values: BaseModel) -> type[T]:
    ...
    @classmethod
    async def paginate(cls, session: AsyncSession, page: int = 0, page_size: int = 10, filters: BaseModel = None):
        filter_dict = filters.model_dump(exclude_unset=True) if filters else {}
        try:
            query = select(cls.model).filter_by(**filter_dict).order_by(desc(cls.model.created_at))
    ...
    async def upsert(cls, session: AsyncSession, unique_fields: List[str], values: BaseModel):
        values_dict = values.model_dump(exclude_unset=False)
